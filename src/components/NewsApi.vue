<template>
  <div class="event-content mt-8 px-6 py-6">
    <button @click="fetchData" class="bg-red-600 h-8 w-32 rounded text-white">
      Fetch
    </button>
    <div
      ...
      <div class="content flex flex-col">
        ...
        <a>
          <div class="flex space-x-2 space-y-2">
            ...
            <div>
              <p
                class="para font-serif text-tiny sm:text-base lg:text-xl text-black"
              >
                {{ formatUnix(article.published) }}
              </p>
            </div>
            <div>
              <p
                class="last-para font-normal font-serif text-tiny sm:text-base xl-text-xl"
              >
                {{ article.source_url }}
              </p>
            </div>
          ...
          <div class="content1 flex flex-row justify-between">
            <div>
              <h2
                class="text-lg sm:text-xl lg:text-2xl font-bold font-serif text-black"
              >
          ...
          <h1 class="font-serif font-medium text-tiny lg:text-xl xl:mb-1">
            {{ article.text }}
          </h1>
        </a>
      </div>
...
export default {
  ...
  methods: {
    async fetchData() {
      let res = await axios
        .get(
          "http://api.mediastack.com/v1/news?access_key=ad9b3abd2ea6d8b7eca54397d22883d3&keywords=christian&countries=ng"
        )
        .then(function (res) {
